/*******************************************************************************
* File Name: check_weight_timer_clock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: check_weight_timer_clock_Start
********************************************************************************
*
* Summary:
...
void check_weight_timer_clock_Start(void) 
{
    ...
    check_weight_timer_clock_CLKEN |= check_weight_timer_clock_CLKEN_MASK;
	check_weight_timer_clock_CLKSTBY |= check_weight_timer_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: check_weight_timer_clock_Stop
********************************************************************************
*
* Summary:
...
void check_weight_timer_clock_Stop(void) 
{
    ...
    check_weight_timer_clock_CLKEN &= (uint8)(~check_weight_timer_clock_CLKEN_MASK);
	check_weight_timer_clock_CLKSTBY &= (uint8)(~check_weight_timer_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: check_weight_timer_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((check_weight_timer_clock_CLKEN & check_weight_timer_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = check_weight_timer_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = check_weight_timer_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* check_weight_timer_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(check_weight_timer_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(check_weight_timer_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: check_weight_timer_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: check_weight_timer_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = check_weight_timer_clock_GetSourceRegister();
    uint16 oldDivider = check_weight_timer_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = check_weight_timer_clock_CLKEN & check_weight_timer_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(check_weight_timer_clock_DIV_PTR, clkDivider);
                check_weight_timer_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                check_weight_timer_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(check_weight_timer_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((check_weight_timer_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(check_weight_timer_clock_DIV_PTR, clkDivider);
				check_weight_timer_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: check_weight_timer_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(check_weight_timer_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: check_weight_timer_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    check_weight_timer_clock_MOD_SRC |= modeBitMask & (uint8)check_weight_timer_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: check_weight_timer_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    check_weight_timer_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(check_weight_timer_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: check_weight_timer_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return check_weight_timer_clock_MOD_SRC & (uint8)(check_weight_timer_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: check_weight_timer_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = check_weight_timer_clock_GetDividerRegister();
    uint8 oldSrc = check_weight_timer_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        check_weight_timer_clock_MOD_SRC |= CYCLK_SSS;
        check_weight_timer_clock_MOD_SRC =
            (check_weight_timer_clock_MOD_SRC & (uint8)(~check_weight_timer_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: check_weight_timer_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return check_weight_timer_clock_MOD_SRC & check_weight_timer_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: check_weight_timer_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    check_weight_timer_clock_PHASE = clkPhase & check_weight_timer_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: check_weight_timer_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return check_weight_timer_clock_PHASE & check_weight_timer_clock_PHASE_MASK;
}
