...
def trigger_state_update(connection: sqlite3.Connection) -> None:
    ...
    try:
        ...
        print(curr_time_ms)
        cursor.execute(
            "UPDATE State SET int_value = ? WHERE key = 'last_update'", (curr_time_ms,)
        )
...
def set_state_out_of_order(
    connection: sqlite3.Connection, message: str, reason: int
) -> None:
    cursor = connection.cursor()
    ...
    try:
        ...
        cursor.execute(
            "UPDATE State SET str_value = ? WHERE key = 'out_of_order_message'",
            (message,),
        )
        cursor.execute(
            "UPDATE State SET int_value = ? WHERE key = 'out_of_order_reason'",
            (reason,),
        )
        connection.commit()
...
def clear_state_out_of_order(connection: sqlite3.Connection) -> None:
    ...
    try:
        ...
        cursor.execute(
            "UPDATE State SET int_value = 0 WHERE key = 'out_of_order_reason'"
        )
        connection.commit()
