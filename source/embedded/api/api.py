...
class Api:
    ...
    def register_routes(self):
        ...
        containers.add_url_rule("/<int:id>",  "change_containers", lambda id: change_containers(id, self.controller),  methods=["PATCH"])
        containers.add_url_rule("/",  "get_containers", lambda: get_containers(self.controller),  methods=["GET"])
        self.app.register_blueprint(containers)

        #Routes for drinks
        drinks = Blueprint("v1/drinks", __name__, url_prefix="/v1/drinks")
        drinks.add_url_rule("/", "create_drink",  lambda: create_drink(self.controller), methods=["POST"])
        drinks.add_url_rule("/<int:id>", "delete_drink", lambda id: delete_drink(id, self.controller), methods=["DELETE"])
        ...
        fluids.add_url_rule("/", "get_fluids",  lambda: get_fluids(self.controller), methods=["GET"])
        fluids.add_url_rule("/", "create_fluid",  lambda: create_fluid(self.controller), methods=["POST"])
        fluids.add_url_rule("/<int:id>",  "delete_fluid", lambda: delete_fluid(id, self.controller), methods=["DELETE"])
